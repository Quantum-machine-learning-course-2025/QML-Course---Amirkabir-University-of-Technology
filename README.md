# Quantum Machine Learning – Fall 2025  
**Amirkabir University of Technology**

---

## 🧠 Course Overview
This repository contains the course materials, lecture plans, and assignments for the **Quantum Machine Learning** course offered in **Fall 2025** at **Amirkabir University of Technology**.

- **Instructor:** Dr. Negar Ashari Astani  
- **Teaching Assistants:** Nima Yadollahi & Alireza Khalaji  
- **Contact:** [qml.ta2025@gmail.com](mailto:qml.ta2025@gmail.com)  
- **Course Level:** Graduate  
- **Course Duration:** 90 minutes per session  
- **Primary References:**  
  - *Machine Learning with Quantum Computers* — Maria Schuld & Francesco Petruccione  
  - [Download Reference Book (PDF)](http://103.203.175.90:81/fdScript/RootOfEBooks/E%20Book%20collection%20-%202025%20-%20F/AI%20and%20DS/Francesco_Petruccione,_Maria_Schuld_Machine_Learning_with_Quantum.pdf)

---

## ⚙️ Requirements

Before starting, students are expected to have a basic understanding of:

- **Python Programming**
- **Quantum Computing**

Recommended reading materials:
- [Introduction to Classical and Quantum Computing – Thomas Wong (PDF)](https://www.thomaswong.net/introduction-to-classical-and-quantum-computing-1e4p.pdf)
- [Quantum Computation and Quantum Information – Nielsen & Chuang (PDF)](https://profmcruz.wordpress.com/wp-content/uploads/2017/08/quantum-computation-and-quantum-information-nielsen-chuang.pdf)

---

## 🧩 TA Class Schedule

| # | Topic | Delivery Mode | Duration | TA |
|---|-------|----------------|-----------|----|
| 1 | Preprocessing & Data Preparation, Regression (Linear Models) | Lesson & Coding | 90 min | Nima |
| 2 | Decision Tree, Random Forests (Tree-based Models) | Lesson & Coding | 90 min | Alireza |
| 3 | Multilayer Perceptron, RNN (Artificial NN) | Lesson & Coding | 90 min | Nima |
| 4 | Convolutional Neural Network (Artificial NN) | Lesson & Coding | 90 min | Alireza |
| 5 | SVM (Kernel Method) | Lesson & Coding | 90 min | Nima |
| 6 | K-Nearest Neighbour (Kernel Method) | Lesson & Coding | 90 min | Alireza |
| 7 | Boltzmann Machine & RBM (Artificial NN) | Lesson & Coding | 90 min | Nima |
| 8 | Hopfield Model, Bayesian Network (Graphical Model) | Lesson & Coding | 90 min | Alireza |

---

## 🧪 Assignments

Assignments are designed to reinforce key quantum and classical machine learning concepts through implementation and experimentation.

| # | Title | Points |
|---|--------|--------|
| 1 | Stack Overflow Salary Survey | 10 |
| 2 | Medical Cost Personal Dataset (Regression) & Cancer Dataset (SVM) | 20 |
| 3 | Code What Was Taught in School | 10 |
| 4 | Apply MLP and RNN on Stack Overflow Salary Survey & Compare with Regression | 20 |
| 5 | COVID CXR Image Dataset | 20 |
| 6 | Grapevine Leaves Image Dataset | 20 |

📦 **Datasets:**
- [Stack Overflow Annual Developer Survey 2024](https://www.kaggle.com/datasets/berkayalan/stack-overflow-annual-developer-survey-2024?resource=download)
- [Medical Cost Personal Dataset](https://www.kaggle.com/datasets/mirichoi0218/insurance)
- [Cancer Data](https://www.kaggle.com/datasets/erdemtaha/cancer-data/data)
- [Great School Dataset](https://www.kaggle.com/datasets/bernardnm/great-school)
- [COVID CXR Image Dataset](https://www.kaggle.com/datasets/sid321axn/covid-cxr-image-dataset-research)
- [Grapevine Leaves Image Dataset](https://www.kaggle.com/datasets/muratkokludataset/grapevine-leaves-image-dataset)

---

## 💻 Related Links

- 🎓 **GitHub Classroom:** [Quantum Machine Learning Course 2025](https://classroom.github.com/classrooms/231667307-quantum-machine-learning-course-2025)  
- 📘 **GitHub Repository:** [QML - Course Repository](https://github.com/Quantum-machine-learning-course-2025/QML-Course---Amirkabir-University-of-Technology)  
- ▶️ **YouTube Playlist:** [Quantum Machine Learning Lectures](https://youtube.com/playlist?list=PL529eKJTemlBZqi900uIeohSd7qqUEG9U&si=Xq-taFyx1A-p0yDL)

---

## 📞 Contact

For any questions or support, please contact:  
📧 **https://qucal.aut.ac.ir/**

---

### © 2025 Amirkabir University of Technology  
**Department of Physics** – Quantum Machine Learning Course
